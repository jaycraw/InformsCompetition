...
spec_cluster = settings_dict["specific_cluster"]["Value"]
if type(spec_cluster) != float:
    spec_cluster = int(spec_cluster)
...
df["months_since_transactions"] = (asofdate - df["business_date"]) / np.timedelta64(1, 'M')
df["time_weighting"] = 1 / ((1+decayfactor)**df["months_since_transactions"])
print("Imports Complete")

#--------------------------------Time Cluster Imports---------------------------------------#
if spec_cluster != "":
    dt_cluster_df = pd.read_csv("user_dt_clusters.csv", dtype=str)
...
#If only looking at specific date cluster per date prediction model, filtering out only those users with that dt
if math.isnan(spec_cluster) == False:
    final_rec_df = final_rec_df.merge(dt_cluster_df[["token_primary_account_identifier","PredictedCluster"]], how="inner", on="token_primary_account_identifier")
    final_rec_df = final_rec_df[final_rec_df["PredictedCluster"]==str(spec_cluster)]
if type(spec_prod) == float:
...
#----------------------------------------Evaluate Accuracy---------------------------------------#
if measure_accuracy == 1 and type(spec_prod) == float:
    ...
    next_transaction = df_val[df_val["business_date"]>asofdate].drop_duplicates(subset="dw_gc_header").sort_values("business_date",ascending=True).groupby("token_primary_account_identifier").head(1)[["token_primary_account_identifier","dw_gc_header"]]
